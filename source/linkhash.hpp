...
public:
	...
	void addItem( H item, const unsigned int key, bool addToTail =false );
	...
	H* getNext() { m_current = m_current ? m_current->nextIter : 0; return getCurrent(); }
	H getNextItem() { m_current = m_current ? m_current->nextIter : 0; return getCurrentItem(); }
...
public:
	...
	public:
		...
		H* getNext() { m_current = m_current ? m_current->nextIter : 0; return m_current ? &(m_current->item) : 0; }
		H getCurrentItem() { return m_current ? m_current->item : 0; }
		H getFirstItem() { m_current = m_list ? m_list->m_head : 0; return m_current ? m_current->item : 0; }
		H getNextItem() { m_current = m_current ? m_current->nextIter : 0; return m_current ? m_current->item : 0; }
		...
		unsigned int getCurrentKey() { return m_current ? m_current->key : 0; }
		void iterate( CLinkHash<H> const& list ) { m_list = &list; m_current = m_list->m_head; }
...
//-----------------------------------------------------------------------------
template<class H> void CLinkHash<H>::addItem( H item, const unsigned int key, bool addToTail /*=false*/ )
{
	Node *N = new Node;
